...
export const handleLitecoinRequest = async (
  ...
  switch (method) {
    ...
    case 'transfer': {
      ...
      const pubkeys = wallet.getSendPubkeys(ChainToNetworkId[Chain.Litecoin], params[0]?.accountIndex);
      if (!pubkeys || pubkeys.length === 0) throw Error('Failed to locate pubkeys for Litecoin');
      ...
      if (result.success && response.unsignedTx) {
        ...
        const signedTx = await sdk.btc.btcSignTransaction({ ...response.unsignedTx, coin: 'Litecoin' });
