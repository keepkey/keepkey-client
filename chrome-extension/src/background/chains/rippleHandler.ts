...
export const handleRippleRequest = async (
  ...
  const tag = TAG + ' | handleRippleRequest | ';
  switch (method) {
    case 'request_accounts': {
      const pubkeys = wallet.getPubkeys(ChainToNetworkId[Chain.Ripple]);
    ...
    case 'transfer': {
      ...
      const pubkeys = wallet.getPubkeys(ChainToNetworkId[Chain.Ripple]);
      if (!pubkeys || pubkeys.length === 0) throw Error('Failed to locate pubkeys for Ripple');
      ...
      if (result.success && response.unsignedTx) {
        ...
        const signedTx = await sdk.ripple.xrpSignTransaction(response.unsignedTx);
